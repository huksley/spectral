...
describe(`Rule '${ruleName}'`, () => {
  ...
  test('return result if components.messages.{message}.payload.examples.{position} is not valid against the schema it decorates', async () => {
    ...
    clone.components.messages.aMessage.payload.examples[1] = { deprecated: 17 };
    ...
    expect(results).toEqual([
      ...
      expect.objectContaining({
        ...
        path: ['components', 'messages', 'aMessage', 'payload', 'examples', '1', 'deprecated'],
        severity: rule.severity,
      }),
    ]);
  });
  ...
  test('return result if components.messageTraits.{trait}.payload.examples.{position} is not valid against the schema it decorates', async () => {
    ...
    clone.components.messageTraits.aTrait.payload.examples[1] = { deprecated: 17 };
    ...
    expect(results).toEqual([
      ...
      expect.objectContaining({
        code: ruleName,
        message: '`deprecated` property type should be boolean',
        path: ['components', 'messageTraits', 'aTrait', 'payload', 'examples', '1', 'deprecated'],
        severity: rule.severity,
      }),
    ]);
  });
  ...
  test.each(['publish', 'subscribe'])(
    ...
    async (property: string) => {
      ...
      clone.channels['users/{userId}/signedUp'][property].message.payload.examples[1] = { deprecated: 17 };
      ...
      expect(results).toEqual([
        ...
        expect.objectContaining({
          code: ruleName,
          message: '`deprecated` property type should be boolean',
          path: ['channels', 'users/{userId}/signedUp', property, 'message', 'payload', 'examples', '1', 'deprecated'],
          severity: rule.severity,
        }),
      ]);
    },
